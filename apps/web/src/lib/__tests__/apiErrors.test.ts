...
describe("apiErrors", () => {
  describe("createErrorResponse", () => {
    ...
    it("should include detail when provided", async () => {
  ...
  describe("createSuccessResponse", () => {
    it("should create a standard success response with default status 200", async () => {
      const response = createSuccessResponse({ value: "test" });
      const body = (await response.json()) as ApiSuccessResponse<{ value: string }>;

      expect(response.status).toBe(200);
      expect(body.ok).toBe(true);
      expect(body.data).toEqual({ value: "test" });
      expect(body.data.value).toBe("test");
    });
    ...
    it("should wrap multiple fields in data", async () => {
  ...
  describe("handleSupabaseError", () => {
    ...
    it("should use fallback code for unknown errors", async () => {
      ...
      expect(body.detail).toContain("Something went wrong");
      expect(body.detail).toContain("unknown_error");
    });
