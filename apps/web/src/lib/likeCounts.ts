...
type SupabaseClientLike = Awaited<ReturnType<typeof supabaseServer>>;

/** Count advert_likes rows per advert id (public-read RLS). Absent ids = 0. */
export async function resolveLikeCounts(
  advertIds: string[],
  options: { cap?: number; supabase?: SupabaseClientLike } = {},
  ...
  // PERF-05: reuse a caller-supplied client (e.g. the home page's shared
  // per-request client) instead of creating a new one when not needed.
