...
export async function apiFetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
  ...
  if (response.status === 429) {
    ...
    // Поддержка как стандартизированного формата {ok: false, error: 'RATE_LIMITED', detail?: string}
    // так и старого формата для обратной совместимости
    const errorLabel = getStringField(payload, "error") || (typeof payload === "object" && payload !== null && "error" in payload ? String(payload.error) : null);
