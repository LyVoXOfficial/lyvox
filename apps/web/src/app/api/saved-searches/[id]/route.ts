...
async function deleteSaved(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!uuidSchema.safeParse(id).success) {
    return createErrorResponse(ApiErrorCode.BAD_INPUT, { status: 400, detail: "Invalid saved search ID" });
  }
  ...
  if (!count) return createErrorResponse(ApiErrorCode.NOT_FOUND, { status: 404, detail: "Saved search not found" });
  return createSuccessResponse({ id });
}

async function patchSaved(request: Request, context: { params: Promise<{ id: string }> }) {
  const { supabase, user } = await getRequestContext(request);
  if (!user) return createErrorResponse(ApiErrorCode.UNAUTH, { status: 401, detail: "Authentication required" });
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) return handleSupabaseError(error, ApiErrorCode.INTERNAL_ERROR);
  if (!data) return createErrorResponse(ApiErrorCode.NOT_FOUND, { status: 404, detail: "Saved search not found" });
