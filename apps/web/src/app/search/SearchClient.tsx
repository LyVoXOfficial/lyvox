...
import type { SearchApiItem } from "@/lib/advertCards";
import {
...
/**
 * SSR seed handed down from the server component (app/search/page.tsx). The
 * server ran the first-page search (+ outside-radius / relaxed fallbacks) so the
 * results are in the initial HTML; this island hydrates from it and only
 * re-fetches client-side on pagination / filter / sort changes.
 *
 * `hydrated`: true when the SSR search produced a real seed. false = the SSR
 * search failed (bad params / transient error) and this island must fetch on
 * mount instead of trusting the (empty) seed.
 * `key`: canonical request signature the seed corresponds to — the fetch guard
 * skips the mount fetch while the URL still matches it.
 */
export type SearchInitialState = {
  ...
  hasMore: boolean;
  page: number;
  outsideItems: SearchApiItem[];
  relaxedItems: SearchApiItem[];
  relaxedTotal: number;
};

/**
 * SearchClient — interactive search UI.
 * Displays results with filters, sorting, and pagination; keeps the URL in sync
 * via the native History API (no server round-trip on filter/sort/page change),
...
export default function SearchClient({
  ...
  // Update the URL without a server round-trip (App Router integrates the native
  // History API with useSearchParams). This is what keeps filter/sort/page
  // changes on the client-fetch path instead of re-running the RSC + double-
  // fetching. scroll defaults to top (parity with the old router.push), except
  // for infinite-scroll appends which must not jump.
  const pushParams = useCallback(
    (queryString?: string, opts?: { scroll?: boolean }) => {
      window.history.pushState(null, "", searchHref(queryString));
  ...
  // State — seeded from the SSR payload so the first paint already has results.
  const [results, setResults] = useState<SearchResult[]>(() => initial.items.map(mapSearchItemToCard));
  const [loading, setLoading] = useState(!initial.hydrated);
  const [error, setError] = useState<string | null>(null);
  const [total, setTotal] = useState(initial.total);
  const [outsideRadiusResults, setOutsideRadiusResults] = useState<SearchResult[]>(() =>
    initial.outsideItems.map(mapSearchItemToCard),
  );
  const [relaxedResults, setRelaxedResults] = useState<SearchResult[]>(() =>
    initial.relaxedItems.map(mapSearchItemToCard),
  );
  const [relaxedTotal, setRelaxedTotal] = useState(initial.relaxedTotal);
  const [hasMore, setHasMore] = useState(initial.hasMore);
  const [filtersOpen, setFiltersOpen] = useState(false);
  const [isMobile, setIsMobile] = useState(false);

  // Fetch guard: the request signature currently reflected in state. Starts at
  // the SSR key (when hydrated) so the mount effect skips the redundant fetch;
  // an empty string when not hydrated forces the mount fetch.
  const lastLoadedKeyRef = useRef(initial.hydrated ? initial.key : "");
  ...
  // Analytics: SEARCH_PERFORMED. Fired once from the SSR seed on mount (the mount
  // fetch is skipped, so fetchResults won't fire it for the initial view); every
  // subsequent client search fires it from fetchResults below.
  const trackSearchPerformed = useCallback(
    (resultCount: number) => {
      const hasFilters = Boolean(
        categoryId || priceMin || priceMax || location || verifiedOnlyFilter || condition,
      );
    ...
    [categoryId, priceMin, priceMax, location, verifiedOnlyFilter, condition, sortBy],
  );
  ...
  const fetchResults = useCallback(async () => {
    ...
    try {
      const baseRequest = readSearchRequestFromParams(
      ...
      if (page === 0) {
        trackSearchPerformed(data.total);
      }
    } catch (err) {
  ...
  }, [page, lat, lng, limit, translate, searchParams, trackSearchPerformed]);

  // Fetch results when the URL changes — but skip while the URL still matches the
  // signature already in state (the SSR seed on mount, or the last client fetch).
  useEffect(() => {
    const currentKey = buildSearchRequestParams(
      readSearchRequestFromParams(new URLSearchParams(searchParams.toString()), limit),
    ).toString();
    if (currentKey === lastLoadedKeyRef.current) return;
    let cancelled = false;
    void fetchResults().then(() => {
      if (!cancelled) lastLoadedKeyRef.current = currentKey;
    });
    return () => {
      cancelled = true;
    };
  }, [fetchResults, searchParams, limit]);

  // Fire SSR-seed analytics once on mount (fetchResults is skipped for the seed).
  useEffect(() => {
    if (analyticsFiredRef.current) return;
    if (!initial.hydrated) return; // fetchResults will fire it instead
    if (initial.page !== 0) return; // matches the page === 0 gate
    analyticsFiredRef.current = true;
    trackSearchPerformed(initial.total);
  }, [initial.hydrated, initial.page, initial.total, trackSearchPerformed]);
  ...
  const [allResults, setAllResults] = useState<SearchResult[]>(() =>
    initial.items.map(mapSearchItemToCard),
  );
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0]?.isIntersecting) {
          ...
          pushParams(params.toString(), { scroll: false });
        }
      },
      { rootMargin: "300px" },
    );
  ...
  }, [isMobile, hasMore, loading, isLoadingMore, page, pushParams, searchParams]);
          ...
          {!loading && !error && total === 0 && outsideRadiusResults.length === 0 && relaxedResults.length === 0 && (
            ...
            >
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {activeFilterChips.length > 0 && (
                  <Button variant="outline" size="sm" onClick={() => pushParams()}>
