-- Improve FTS ranking: title weighting (A) + description (B) + length-normalised
-- ts_rank_cd + deterministic `id` tiebreaker on all sort branches.
-- Signature is IDENTICAL to 20260626120000 (same 13 params) — safe to push while
-- the deployed app code is already live. Using 'simple' on BOTH sides so stemmed
-- tokens never fail to match the query vector.
...
begin
      ...
      case
        when query_tsquery is not null then
          ts_rank_cd(
            ...
            query_tsquery,
            1  -- normalise by document length
          )
        else 0.0
  ...
  order by
    ...
    f.created_at desc,
    f.id  -- deterministic tiebreaker prevents row drift across offset pages
  offset page_offset
  limit page_limit;
end;
$$;
