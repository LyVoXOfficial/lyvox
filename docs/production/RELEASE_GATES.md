...
| GitHub CI          | Последние 20 запусков падают до checkout из-за невалидных action SHA                                     | Красный |
| Защита main        | Branch protection отсутствует, production deploy не зависит от green CI                                  | Красный |
| Supply chain       | `pnpm audit --prod`: 1 critical, 13 high, 13 moderate, 2 low                                             | Красный |
...
| Cron               | Root и app Vercel configs задают разные расписания                                                       | Красный |
...
- [ ] Все GitHub Actions используют существующие immutable SHA.
- [ ] Gitleaks тоже закреплён SHA, mutable tag отсутствует.
- [ ] Required CI checks защищают `main`.
- [ ] Production deployment не идёт при красном required check.
- [ ] PR review и запрет force-push настроены для `main`.
...
- [ ] Нет exploitable critical/high production advisory.
...
- [ ] Lockfile immutable в CI; package provenance и Dependabot включены.
- [ ] Audit job проверяет structured result, а не grep по слову `next`.
