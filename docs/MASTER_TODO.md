> [!WARNING]
> **Исторический снимок от 2026-06-28. Не использовать как бэклог, не выполнять задачи по порядку ниже и не обновлять статусы.** Единственный актуальный источник задач, release-gates и production-статуса: [`docs/MASTER_PRODUCTION_TZ.md`](./MASTER_PRODUCTION_TZ.md). Ссылки на PRD сохранены только как архивный контекст и перед использованием должны быть подтверждены в master-документе.

# LyVoX — MASTER TODO / ТЗ (исторический архив)

Дата снимка: 2026-06-28 · На эту дату файл использовался как рабочий backlog; с 2026-07-10 он полностью заменён `MASTER_PRODUCTION_TZ.md`. Каждый пункт ниже ведёт к историческому детальному отчёту (PRD/аудит), но его статус и необходимость требуют повторной проверки по актуальному master-документу.
...
| B5 | ~~**`pnpm gen:types` нужен перед деплоем**~~ ✅ **ЗАКРЫТ** — `database.types.ts` перегенерирован после prod schema push; `analytics_events`, `viewer_key`, `view_hour`, `trust_score.last_computed_at`, `discover_prefs`, `itsme_sub`, `webhook_events` теперь проходят типизацию; B5-специфичные временные casts/comments убраны. | — |
