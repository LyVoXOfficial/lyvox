> [!CAUTION]
> **Архивный checklist. Не выполнять, не отмечать галочки и не запускать скрипты, которые обновляют этот файл.** Единственный рабочий backlog и production-gate: [`docs/MASTER_PRODUCTION_TZ.md`](../MASTER_PRODUCTION_TZ.md). Файл сохранён из-за исторических ссылок.

# Master Checklist - LyVoX Development

> **АРХИВНАЯ ФОРМУЛИРОВКА, БОЛЬШЕ НЕ ДЕЙСТВУЕТ:** раньше этот файл назывался главным списком задач команды.

> **Архивная интеграция отключена:** `PROMPT_MAIN.md` и автоматическое обновление прогресса больше не используются. Не запускать старый updater и не менять этот файл как текущий tracker.
