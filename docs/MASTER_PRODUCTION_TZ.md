...
**🟡 Follow-ups (записаны, вне scope шага):** (a) `media(advert_id,url)` unique index + `INSERT … ON CONFLICT` (конкурентные `/complete` → дубль-строки + обход cap; предсуществующая гонка, рискованно на prod-дубликатах); (b) orphan-sweep cron в `maintenance-cleanup` для брошенных upload-ов (клиент может залить и не звать `/complete`); (c) in-process concurrency-семафор на sanitize (→ `SEC-DoS`, шаг 32). **Прод-верификация:** linux-x64 sharp-бинарь резолвится только в deploy-build (RULE-04, локально win32) — проверить билд-лог при деплое.
