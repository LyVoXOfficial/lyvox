# LyVoX: MASTER PRODUCTION ТЗ

Версия: 2.4
Обновлено: 2026-07-11
Владелец продукта: founder LyVoX
Статус: **единственный рабочий источник целевого состояния, приоритетов и текущего Production-статуса**

README, CLAUDE и все подчинённые PRD обязаны ссылаться сюда. Удалённые legacy trackers/checklists больше не являются источниками задач или статусов. Сохранённые `docs/todo/*`, historical plans, audits и implementation notes являются только архивными/подчинёнными справками: они не активируют работу, не задают priority/status/order и не могут переопределить этот master. `PROJECT_VISION_AND_TZ` описывает продуктовую идею, а `docs/features/*` уточняют поведение, но не меняют приоритет или статус из этого мастера.

При конфликте:

1. фактическое текущее поведение подтверждают код, миграции и воспроизводимый runtime evidence;
2. целевое состояние и порядок задаёт только этот мастер;
3. release criteria задаёт [production/RELEASE_GATES.md](production/RELEASE_GATES.md);
4. активацию функций задаёт [production/CAPABILITY_ACTIVATION_MATRIX.md](production/CAPABILITY_ACTIVATION_MATRIX.md);
5. визуальную систему задаёт [DESIGN_SYSTEM.md](DESIGN_SYSTEM.md);
6. работу моделей задаёт [production/AGENT_EXECUTION_PROTOCOL.md](production/AGENT_EXECUTION_PROTOCOL.md);
7. [aegis/plans/2026-07-10-production-readiness-v2.md](aegis/plans/2026-07-10-production-readiness-v2.md) содержит только подчинённые технические заметки о файлах и проверках; он не задаёт priority/status/order и не активирует задачи вне этого master.

## 0. Определение «готово»

Production-готовность не означает, что все будущие функции включены. Она означает, что core работает и доказан, а каждая внешняя функция находится в одном честном состоянии:

- `VERIFIED + ON/READY`: реально проверена и может работать;
- `CODE_COMPLETE + BLOCKED_KEYS`: код готов, ждёт provider credentials;
- `CODE_COMPLETE + BLOCKED_LEGAL`: код/симулятор готов, но live use запрещён;
- `OFF/EMERGENCY_OFF`: безопасно недостижима.

Нельзя считать disabled adapter, mock или наличие тумблера работающей интеграцией.

### Две оси статуса

**Implementation:** `NOT_STARTED` → `IN_PROGRESS` → `CODE_COMPLETE` → `VERIFIED`.
**Activation:** `OFF` · `BLOCKED_KEYS` · `BLOCKED_LEGAL` · `BLOCKED_RELEASE` · `READY` · `ON` · `DEGRADED` · `EMERGENCY_OFF`.

`CODE_COMPLETE_NOT_VERIFIED` — отчётная пометка: реализация заявленного scope закончена, но обязательный runtime/migration/release evidence ещё не получен; это не `VERIFIED` и не разрешение на активацию.

Старые символы ✅/🟡/⛔ в архивной части ниже являются историческим срезом и не закрывают V2 gate.

### Золотой принцип активации
...
- Секреты остаются в защищённом provider secret store, не в БД и не в браузерной админке.
- `platform_settings` хранит только желаемый toggle, безопасный config, revision и ссылки на approval evidence.
- Удаление ключа блокирует capability без 500.
- Для money, identity, admin и legal-sensitive функций ошибка config store означает OFF.
- UI, API, webhook, cron и background worker проверяют один server-side effective resolver.

## 1. Режимы выпуска

| Режим                          | Что считается выпущенным                                                                      | Главный внешний блокер                      |
| ------------------------------ | --------------------------------------------------------------------------------------------- | ------------------------------------------- |
| `R0 Technical Ready`           | Код, админка, закрытый production-rehearsal, CI, E2E, monitoring, recovery и dormant adapters | Нет компании не блокирует                   |
| `R1 Contact-only Production`   | Listings, search, profile, chat, reports, moderation; денег нет                               | Утверждённый operator/legal/support package |
| `R2 Own Revenue`               | Pro, boosts и собственные сборы LyVoX                                                         | Company, VAT/accounting, Stripe, terms      |
| `R3 Contracted Integrations`   | itsme, Stripe Identity, WhatsApp, KYBC, Bpost и другие контракты                              | Provider contract/DPA/company               |
| `R4 Transactional Marketplace` | Оплата товара, escrow, payout, disputes и связанная доставка                                  | F2/F3, PSD2/AML/KYC и письменный sign-off   |

До регистрации компании главная цель: полностью закрыть R0. В текущем `contact_only` Stripe/escrow/payout paths должны быть технически недостижимы даже при случайно заданных live keys.

### 1.1. Решение основателя: закрытый Production-as-Staging

До публичного запуска LyVoX разворачивается и тестируется прямо на production-инфраструктуре, потому что проект ещё не анонсирован и реального пользовательского трафика нет. Это **закрытая production-rehearsal среда**, а не объявление публичного `R1` и не отказ от release gates.

Обязательный контур этого режима:

- неавторизованный посетитель видит только аккуратную `Coming Soon`-страницу с кратким честным описанием проекта и входом для команды;
- доступ к самому продукту проверяется server-side access gate: команда вводит отдельный invitation code, после успешной Turnstile/rate-limit проверки браузер получает короткоживущую подписанную `HttpOnly + Secure + SameSite=Lax` cookie без исходного кода внутри; скрытие навигации на клиенте не считается защитой;
- production и Vercel Preview закрыты по умолчанию; Preview нельзя открыть копированием production-флага. Для закрытого production `PRODUCTION_ACCESS_GATE_ENABLED` обязан быть `true` либо отсутствовать; значение `false/0` разрешено только как часть отдельного go/no-go снятия gate;
- unlock остаётся fail-closed, пока одновременно не заданы `PRODUCTION_ACCESS_CODE` (не менее 16 bytes), отдельный `PRODUCTION_ACCESS_GATE_SECRET` (не менее 32 bytes), `NEXT_PUBLIC_TURNSTILE_SITE_KEY`, `TURNSTILE_SECRET_KEY`, `UPSTASH_REDIS_REST_URL` и `UPSTASH_REDIS_REST_TOKEN`;
- безопасный rollout выполняется только в порядке `provider secrets/config → закрытый deploy exact SHA → внешний smoke без cookie → founder/team access → дальнейшие DB/runtime rehearsal`. До доступа команды чистый браузер обязан видеть только `Coming Soon`, обычные API — `401/403`, sitemap — `404`, закрытый `robots.txt`, а unlock/relock и собственная авторизация allowlisted machine endpoints должны быть проверены отдельно;
- публичная заглушка имеет `noindex, nofollow`; product sitemap/catalog и регистрация для произвольных посетителей не публикуются до снятия gate;
- API, admin, webhook и cron сохраняют собственные auth/signature/secret checks: общий access gate не является заменой их авторизации;
- effective launch mode остаётся `contact_only`, global money stop — `ON`, а `paid_boosts`, Pro, SMS/WhatsApp, identity и marketplace money — `OFF/BLOCKED_*`; наличие случайно вставленных ключей не может включить их без полного server-side AND-gate;
- seed-витрина сохраняется; реальные клиенты, реальные платежи и необратимые бизнес-операции в закрытом режиме запрещены;
- production-БД всё равно считается source of truth: миграции, backup/restore, rollback и audit не упрощаются только потому, что доступ закрыт;
- снять `Coming Soon`/access gate можно только отдельным go/no-go решением после закрытия соответствующих `R0/R1` gates и фиксации evidence на одном exact SHA.

Это решение заменяет требование отдельной hosted staging-среды для текущего pre-launch цикла. Локальная disposable БД/реплика остаётся обязательной для чистого replay миграций и destructive/security rehearsal; Shannon никогда не запускается против production.

## 2. Scope R1: честный contact-only продукт

Входит:

- регистрация, вход, восстановление, consent и профиль;
- создание, черновик, редактирование, публикация и удаление объявления;
- категории, поиск, фильтры, сортировка, detail;
- favourites, saved search и in-app уведомления;
- contact/chat, block/report и антифрод-предупреждения;
- ручная модерация, appeal/contact и admin audit;
- privacy export/delete;
- EN/FR/NL/DE/RU, accessibility, mobile/desktop, light/dark;
- monitoring, incident response, backup/restore и release rollback.

Не входит и остаётся недостижимым до своих gates:

- принятие денег за товар, escrow, payout;
- обещание identity/business verification при неактивном provider;
- автоматическое финальное решение AI без human appeal;
- интегрированная доставка с финансовой ответственностью;
- Pro/boost billing до R2.

## 3. Baseline 2026-07-10: что расширено относительно старого ТЗ

| Найденный blocker                             | Почему старых задач не хватало                   | Новый gate                            |
| --------------------------------------------- | ------------------------------------------------ | ------------------------------------- |
| GitHub CI падает до checkout; main не защищён | `OPS-CI` был P1 и считался частичным             | P0 repository/deploy governance       |
| 1 critical + 13 high production advisories    | `SEC-DEP` был одновременно ✅ и ⛔               | P0 dependency/supply-chain gate       |
| CSP остаётся report-only                      | Исторический шаг был зачёркнут как сделанный     | P0 enforced-CSP evidence              |
| Debug/test-auth в production build            | Не было отдельного stop-ship                     | P0 debug-surface removal              |
| Нет health/readiness                          | `OPS-HEALTH` был P1                              | P0 reliability gate                   |
| Playwright фактически не настроен             | `OPS-E2E` был P1                                 | P0 real browser harness               |
| Нет restore drill/production rehearsal        | Backup и pre-launch rehearsal были сгруппированы | Два независимых P0 gates              |
| Runtime settings не используются call sites   | FLAG foundation ошибочно выглядел почти готовым  | P0 registry + migration + audit       |
| Boost checkout не имеет полного money gate    | `boost_ranking` смешивал оплату и алгоритм       | Отдельный `paid_boosts` + launch mode |
| Public claims не соответствуют adapters       | Не было product-truth gate                       | P0 capability-aware copy              |
| Legal pages draft, operator pending           | Legal считался одним внешним пунктом             | R0 templates + R1 approval gate       |
| Email domain не имеет доказанной доставки     | Не было deliverability/support gate              | P0 MX/SPF/DKIM/DMARC + E2E            |
| Cron configs расходятся; cleanup auth опасен  | OPS был одним общим шагом                        | P0 cron manifest/auth/idempotency     |
| Старый дизайн предписывал AI-шаблон           | UX сводился к motion/polish                      | P0 design contract + visual/axe       |
| Shannon не запускался                         | Pentest был P2                                   | P0 pre-customer proof                 |

Полный measured baseline и stop-ship находятся в `production/RELEASE_GATES.md`.

## 4. Полнота по всем направлениям

| Направление        | Целевое состояние                                         | Режим |
| ------------------ | --------------------------------------------------------- | ----- |
| Governance/docs    | Один master, evidence-linked status                       | R0    |
| Product core       | Все critical journeys и failure states E2E                | R0/R1 |
| Design             | Product-first, anti-AI, consistent public/product/admin   | R0    |
| Accessibility      | WCAG 2.2 AA, keyboard, zoom, touch                        | R0    |
| i18n/content       | 5 locales, no raw keys, factual copy                      | R0    |
| Security           | Enforced CSP, RLS/authz, admin MFA, no debug, Shannon     | R0    |
| Privacy/legal      | Templates/code at R0, approved operator package at R1     | R0/R1 |
| Capability control | Registry, launch mode, admin, health, audit, kill switch  | R0    |
| Admin/operations   | Dense shell, queues, settings, audit and partial failures | R0    |
| Auth/identity      | Core auth at R1; contracted identity activation at R3     | R1/R3 |
| Communications     | In-app/email/support ready; SMS/WhatsApp gated            | R1/R3 |
| Revenue            | Own fees only after company/VAT/Stripe                    | R2    |
| Marketplace money  | Simulator/state machine first; live only after F2/F3      | R4    |
| Data/database      | RLS, migrations, backup, restore, retention               | R0    |
| Reliability        | Health, Sentry, uptime, quotas, SLO/error budget          | R0    |
| Background jobs    | One manifest, auth, lock, idempotency, last success       | R0    |
| QA                 | Unit/integration/E2E/visual/axe/security                  | R0    |
| CI/CD              | Protected main and deploy gated by green exact SHA        | R0    |
| Performance/SEO    | CWV/bundle/query budgets, canonical/hreflang/JSON-LD      | R0    |
| Analytics/AI       | Consent/cost/human review; no decorative AI claims        | R0/P1 |
| Support/moderation | Operational owner, abuse escalation and honest SLA        | R1    |
| External services  | Adapter + sandbox/contract + activation runbook           | R0/P1 |

## 5. V2 P0: обязательный порядок до R0

Подчинённые технические notes по файлам и проверкам: `docs/aegis/plans/2026-07-10-production-readiness-v2.md`. Они используются только после активации пункта этим master и не являются отдельным backlog/status/order.

- [x] **P0-00 SSOT.** ✅ `VERIFIED` 2026-07-11: этот master закреплён как единственный canonical backlog/status/order source; конкурирующие trackers/checklists и устаревшие prompts удалены. Реализованные T01–T11/T13–T17/T19 task-contracts удалены; уникальные T12/T18/T20 contracts и T08 evidence сохранены только как явно архивные/подчинённые справки без собственного приоритета или статуса. Живые ссылки переведены сюда, обязательные инварианты сохранены в project rules.
- [ ] **P0-01 CI/MAIN.** ⛔ `CODE_COMPLETE + EXTERNAL NO-GO`: action SHA исправлены, Gitleaks CLI закреплён с checksum, `main` защищён active ruleset, автоматический Vercel Git deploy для `main` выключен и заменён REST staged→promote workflow после `CI Success` с exact-SHA, project/runtime и incident-race guards. На remote SHA `a8f0d1e` все jobs кроме ожидаемого history secret scan зелёные, Preview прошёл, aggregate gate честно красный. До `VERIFIED` обязательны: (1) ротация двух исторически засвеченных Google API keys, (2) зелёный полный secret/history scan и aggregate CI, (3) Vercel secrets/config и Production promote proof на exact SHA, (4) независимый human reviewer, (5) воспроизводимый hotfix drill. До этого публичный launch — `NO-GO`, допускается только закрытый Production-as-Staging за access gate. Evidence: [production/evidence/P0-01.md](production/evidence/P0-01.md).
- [x] **P0-02 DEPENDENCIES.** ✅ `VERIFIED`: production audit снижен с `1 critical / 13 high / 13 moderate / 2 low` до `0 / 0 / 0 / 0`; полный workspace/toolchain audit также `0 / 0 / 0 / 0`. Exact-SHA remote audit зелёный, независимый verifier подтвердил результат. Evidence: [production/evidence/P0-02.md](production/evidence/P0-02.md).
- [ ] **P0-03 LAUNCH-MODE.** 🟡 `CODE_COMPLETE + LOCAL REPLAY VERIFIED`: реализованы fail-closed `contact_only`, абсолютный money hard-stop, отдельные `paid_boosts`/`boost_ranking`, server-side коммерческий boundary и физические блокировки checkout/products/subscribe/webhook. Опасные settings не доверяют Redis как authority. Актуальный migration head прошёл fresh `supabase db reset --local --no-seed`, production control-plane smoke, DB lint/advisors и независимый SQL review; 2026-07-11 focused production-suite зелёный `276/276`. Не общий `VERIFIED`: обязательны linked read-only preflight, координированный rollout, закрытый production smoke и signed money-stop evidence на exact SHA.
- [ ] **P0-04 FLAGS/ADMIN.** 🟡 `CODE_COMPLETE + LOCAL REPLAY VERIFIED`: registry выдаёт implementation/activation/blocker/approval/health truth; добавлены AAL2-admin settings, revision conflict, immutable audit и атомарный emergency stop/kill switch. Коммерческие и sensitive capabilities fail-closed. Актуальный migration head и authz guard прошли fresh clean replay/smoke/lint/advisors; migration guard зелёный `36/36`, независимый SQL review — `GO` для локального scope. Не общий `VERIFIED`: миграции ещё не применены к production, нет deployed browser/runtime evidence для conflict/stale/emergency recovery.
- [ ] **P0-05 PRODUCT-TRUTH.** 🟡 `CODE_COMPLETE_NOT_VERIFIED`: runtime call sites переведены на effective resolver; публичные payment/Pro/identity/Discover claims скрываются или формулируются по фактической capability; fake boost badge удалён; saved-search cadence сведён к `daily/off`; analytics/error tracking/SMS требуют capability. Не `VERIFIED`: нет полного browser/SEO/a11y/5-locale evidence на закрытом production exact SHA, а P0-10/11/12 остаются `NO-GO`.
- [ ] **P0-06 SECURITY.** 🟡 `IN_PROGRESS / LOCAL DB+GATE VERIFIED`: удалены production debug/test-auth routes; закрыты external `returnTo` redirects и PKCE recovery/reset flow; CSP-report ограничен по size/content-type/rate и пишет только очищенные поля. Access gate default-closed для Production и Vercel Preview: обычные UI/API, sitemap, RSC/segment data, `_rsc`, spoofed middleware headers и encoded-path варианты закрыты server-side; остаётся только точный method/path allowlist отдельно авторизованных machine endpoints. Fresh final artifact: `15/15` adversarial route probes fail-closed, security headers/no-store/noindex подтверждены, `26/26` scripts имеют nonce, browser console/network clean. В БД реализованы history-aware fail-before-DDL baseline, provenance cleanup, browser `TRUNCATE` revocation, schema-contract convergence и hardening всех reviewed `SECURITY DEFINER`; fresh reset, control-plane smoke, DB lint/advisors и guard `36/36` зелёные, независимый SQL review — `GO` для локального scope. Не общий `VERIFIED`: перед закрытым deploy нужны production env/rebuild и реальные Turnstile-hostname + Upstash smoke; linked preflight обязан подтвердить отсутствие `CREATE` у `anon/authenticated` в `public`/`auth` и создание production-таблиц только migration role. Enforced CSP обязателен перед public GO после чистой report-only telemetry; далее остаются live authz/advisor review, Shannon и deployed exact-SHA evidence. До OAuth/прямого Supabase signup нужно миграционно создать и проверить `on_auth_user_created` для `handle_new_user()` либо явно закрепить route-only signup: текущий `/api/auth/register` создаёт profile сам, альтернативные auth-пути — ещё нет.
- [ ] **P0-07 E2E.** Настоящий Playwright harness и deterministic critical journeys в CI.
- [ ] **P0-08 OPS.** Health/readiness, Sentry/uptime/quota alerts, единый cron manifest, quiet-hours fix.
- [ ] **P0-09 DR/REHEARSAL.** ⛔ `IN_PROGRESS / LOCAL REPLAY VERIFIED / BLOCKED_RELEASE`: history-aware clean-only baseline и forward convergence реализованы. Актуальный head 2026-07-11 прошёл fresh `supabase db reset --local --no-seed`, production control-plane/authz smoke, DB lint, advisors, `pg_default_acl` checks и adversarial baseline rerun на established/partial/zero-core-with-history сценариях; независимый review — `GO` для reviewed local slice. Rollout остаётся `NO-GO` до read-only linked schema/migration/default-ACL preflight; managed `supabase_admin` default ACL нельзя считать доказанным локально, поэтому production schema changes через Dashboard/другого owner запрещены, новые таблицы создаются только migration role. Index/schema DDL намеренно non-concurrent и fail-safe ограничен `5s lock_timeout`/`60s statement_timeout`: до push обязательны size/lock-budget preflight и maintenance window. Backdated файлы требуют только координированный `supabase db push --include-all --db-url "$SUPABASE_DB_URL"`, обычный push и `migration repair` для обхода отказов недопустимы. После rollout обязательны DB+Storage restore drill, rollback и signed evidence.
- [ ] **P0-10 DESIGN-FOUNDATION.** ⛔ `DESIGN AUDIT NO-GO`: home/sell/billing/Pro используют узнаваемый AI-шаблон (градиенты, декоративные blobs, одинаковые карточки); обнаружены contrast/token failures и touch targets меньше 44px; отсутствуют visual-regression и axe harness. Нужны устойчивые tokens/primitives, light/dark/reduced-motion, AA contrast и доказанные keyboard/zoom/touch states без универсального «AI-gradient» языка.
- [ ] **P0-11 PUBLIC-DESIGN.** ⛔ `DESIGN AUDIT NO-GO` для продукта в целом: SearchBar не завершён как доступный combobox; billing DB failures маскируются под пустую историю; Pro безусловно показывает shield и неверно маркирует count по первым 20 объявлениям; public copy/layout ещё не везде product-first и factual. Отдельный закрытый `Coming Soon`-экран получил независимый локальный `GO`: редакционный визуальный язык без AI-gradient/card шаблона, factual copy на 5 языках, noindex, skip-link, видимый focus и ≥51px controls; reflow чистый `20/20` (5 локалей × 320/390/768/1440), WCAG text-spacing `5/5`, axe 4.11 WCAG 2.2 AA `10/10` прогонов — 0 violations и 0 incomplete. Это не закрывает full-product redesign и deployed visual evidence. До общего `VERIFIED` обязательны production screenshots и evidence из Facebook Sharing Debugger: для публичного `Coming Soon` URL в закрытом режиме и для canonical advert URL перед снятием access gate (fresh scrape, title, description, image, canonical target).
- [ ] **P0-12 PRODUCT-ADMIN-DESIGN.** ⛔ `DESIGN AUDIT NO-GO`: локальный AAL2 admin header/nav добавлен, но admin всё ещё вложен в public root chrome и страницы создают nested `main`; forms не имеют полного набора label/pending/error/conflict/stale states; BoostDialog click-card не является keyboard/radio control; Pro wizard не закрывает focus/errors/progress/mobile; MFA/admin copy частично English-only. Нужны действительно отдельный плотный admin shell, корректные landmarks, 5-locale states, responsive/keyboard/focus evidence и честные partial-failure экраны.
- [ ] **P0-13 CORE-CLOSURE.** Buyer/seller/moderator/admin journeys, privacy, abuse and recovery.
- [ ] **P0-14 LEGAL-EMAIL-SUPPORT.** Templates, operator data model, DNS/delivery, support/DSA/privacy operations.
- [ ] **P0-15 SHANNON.** Разрешённый proof только на local/disposable production-like replica, reproduce findings, zero confirmed exploitable critical/high; никогда не направлять Shannon на production.
- [ ] **P0-16 R0-REHEARSAL.** Один SHA, все G0-G14, закрытый production deploy за access gate, founder/team smoke, rollback и signed evidence; отдельно зафиксировать go/no-go перед снятием `Coming Soon`.

Нельзя переносить P0-08/P0-09/P0-15 «после запуска». Это часть определения Production.

## 6. P1: activation-ready до/сразу после регистрации

P1 не блокирует contact-only R1, но блокирует заявление владельца «осталось только вставить ключ»:

- [ ] Email outbox/retry/bounce/complaint и provider adapter.
- [ ] Twilio SMS/WhatsApp OTP adapters вместо прямых/disabled paths; активация только после server-side Turnstile на abuse paths, обоих Turnstile keys, полного набора Twilio credentials, Twilio sandbox rehearsal, утверждённого Twilio DPA/approval и provider-health evidence. Текущий SMS статус — `PARTIAL/OFF`, не «вставить ключ и включить».
- [ ] itsme OIDC и Stripe Identity sandbox-ready adapters.
- [ ] Pro/boost/Stripe Tax test-mode, webhook journal и reconciliation. Перед активацией обязательны все пункты ниже:
  - Pro entitlement выдаётся только по доказанному paid state с ожидаемыми price/currency, сохранённым provider subscription ID и событием `invoice.paid` либо эквивалентным строго оплаченным событием;
  - webhook journal получает атомарный claim/lease; transient DB/provider failures возвращаются на retry, permanent failures уходят в наблюдаемый dead-letter/reconciliation path и не ACK-аются молча;
  - purchases хранят immutable offer snapshot через `product_offer_versions` или эквивалентный неизменяемый ledger; boost привязан к конкретным advert/session/payment/amount/currency/metadata и повтор события идемпотентен;
  - subscribe fail-closed проверяет blocked user, подтверждает изменённую profile row, исключает duplicate active subscription и имеет provider idempotency;
  - `NEXT_PUBLIC_SITE_URL` обязателен, localhost fallback в activation-ready flow запрещён;
  - Stripe Tax/address collection и юридическая tax-модель утверждены; health gate доказывает mode, webhook, prices/products, Tax и reconciliation в sandbox.

Пока эти пункты не закрыты, Pro/boost имеют статус `PARTIAL/OFF` и остаются физически недостижимыми через effective capability независимо от наличия Stripe keys.

- [ ] Translation provider с cost/quality controls.
- [ ] AI moderation с schema/timeout/cost/human appeal/DPIA gates.
- [ ] Web Push/VAPID lifecycle.
- [ ] Business/KYBC provider flow и human fallback.
- [ ] Bpost/PUDO sandbox adapter.
- [ ] Payment/deal/dispute/delivery state machines и deterministic PSP simulator без live money.

Если provider не даёт sandbox до регистрации, допустимый статус: `CODE_COMPLETE + BLOCKED_EXTERNAL`, с contract fixtures и готовой smoke-командой.

## 7. P2: после доказанного трафика

- AI listing-from-photo и price assistant;
- referrals/gamification;
- сложная seller analytics;
- расширенный motion сверх functional feedback;
- экспериментальные discovery-механики без подтверждённой ценности;
- масштабирование WAF/HA/observability сверх измеренной потребности.

P2 не может отвлекать от красного P0 gate.

## 8. Работа моделей

| Роль          | Ответственность                                      |
| ------------- | ---------------------------------------------------- |
| Codex Sol     | Архитектура, task contracts, зависимости, синтез     |
| Codex Terra   | Backend/platform/data/security/integrations/ops      |
| Codex Luna    | Независимая verification и release evidence          |
| Claude Sonnet | Атомарная реализация и tests                         |
| Claude Fable5 | Product/design/copy/five-locale и adversarial review |

Security, auth, money и migrations всегда имеют verifier другой модели/семейства. Один файл одновременно имеет одного write-owner. Полный handoff/evidence format находится в `production/AGENT_EXECUTION_PROTOCOL.md`.

## 9. Go/No-Go

Решение по каждому режиму принимается отдельно. R0 не делает сайт коммерчески live. R1 не разрешает деньги. R2 не разрешает escrow. R3 не включает все providers разом. R4 требует F2/F3.

Любой stop-ship из `production/RELEASE_GATES.md` означает NO-GO, даже если локальные tests зелёные.

---

Ниже сохранён детальный инженерный dossier версии 2026-07-04: решения, рассмотренные варианты и история уже выполненных оптимизаций. Он полезен как справочник, но его старый порядок и символы статуса **не являются V2-трекером**.

---

## Архив V1: прежний критический путь

> Не исполнять как текущий порядок. Актуальный порядок находится в разделе «V2 P0» и в Aegis plan выше.
...
## Архив V1: прежняя модельная рубрика

> Назначения Haiku/Opus/Workflow устарели. Текущие роли Sol/Terra/Luna/Sonnet/Fable5 определены в разделе 8 и Agent Execution Protocol.
...
## Архив V1: прежний TODO и история выполнения

**Не использовать как текущий трекер.** Список сохранён, чтобы не потерять evidence уже выполненных работ и старые ID. Новые статусы меняются только в V2-разделах этого мастера.
...
- [x] ~~**04.** `SEC-CSP` — CSP report-only → enforced (nonce + strict-dynamic, убрать `unsafe-inline`/`unsafe-eval`)~~ ✅ **код готов (report-only по умолчанию)** 2026-07-04: nonce-CSP первоначально жил в `middleware.ts`, текущий Next 16 owner — `proxy.ts` + `lib/security/csp.ts`; `unsafe-inline`/`unsafe-eval` сняты, Turnstile/Sentry-хосты добавлены. 🟡 **enforce-flip (`CSP_MODE=enforce`) — гейт на проде** после ревью `/api/csp-report` (RULE-04, см. SEC-CSP) — 🎚️ **[Workflow · ultracode]**
...
- [x] ~~**10.** `SEC-AUTHZ-GUARD` — CI-гард RLS/column-grants/function-grants (фиксирует RULE-01/02 автоматикой)~~ ✅ 2026-07-05 — статик-гард `authz-migration-guard.test.ts` (RULE-A/RULE-01/RULE-02 на новых миграциях, allowlist по образцу SEC-VALID, +positive-detection) едет на `pnpm test`; **живой аудит** `scripts/audit-authz-live.mjs` (`pnpm authz:audit`, `role_table_grants × pg_policies × pg_proc`) прогнан по проду → отчёт `docs/security/SEC-AUTHZ-GUARD-live-audit.md`. Follow-up: column-scope `profiles`-INSERT (verified*\*/itsme*\*/pro_until) + purchases/reports — 🎚️ **[Opus 4.8 · high]**
...
- [ ] **35.** `OPS-HEALTH` + `OPS-E2E` + `OPS-CI` + `OPS-STAGING` + `OPS-BACKUP` + `OPS-ANALYTICS` — health/uptime + Playwright + CI-гейты + закрытый production-rehearsal + бэкапы/DR + дашборд метрик — 🎚️ **[Opus 4.8 · xhigh]** _(разброс: OPS-ERR/HEALTH — Haiku·low; OPS-BACKUP — Opus·xhigh)_

### Фаза H — продуктовая полнота

- [ ] **36.** `PROD-BIZ` + `PROD-TRUST` + `PROD-DISCOVERY` + `PROD-COMPLIANCE` + `PROD-UX-MOBILE` — довести 🟡-фичи (кабинет-по-членству, AI-модерация, swipe/saved-search/PWA-push, Recupel, мобильная раскладка) — 🎚️ **[Opus 4.8 · high]** _(разброс: PROD-UX-MOBILE — Haiku·low; PROD-TRUST — Opus·high)_
...
| nonce-CSP: двойной `Content-Security-Policy` (next.config **и** edge proxy) → браузер пересекает политики → ломает                                                     | 1×                     | CSP живёт РОВНО в одном месте (`proxy.ts`); next.config держит только non-CSP headers                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| nonce-CSP: Next читает nonce из **request**-заголовка `Content-Security-Policy` (не response)                                                                          | 1×                     | В `proxy.ts` ставить `x-nonce`+`Content-Security-Policy` на request headers (имя всегда enforcing, даже в report-only), иначе Next не занонсит bootstrap                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
_(при повторе любой строки до счётчика 3 — переносим в «Правила» с новым RULE-ID и, если применимо, добавляем CI-гард)_
...
**Рассмотрено (10) →** ① остаться report-only — ✗; ② enforce как есть (с unsafe-\*) — ✗ бесполезно; ③ enforce, убрать unsafe-inline, хэши для известных inline — хрупко в Next; ④ **nonce-based `script-src` + `strict-dynamic`, nonce из edge proxy** — ✓ основной; ⑤ hash-based — ✗ сложно с динамикой; ⑥ report-uri.com/внешний коллектор — дополняет как аналитика; ⑦ убрать CSP, полагаться на экранирование — ✗; ⑧ Trusted Types + CSP — сильнейшее, фаза 2; ⑨ nonce + strict-dynamic + Trusted Types + report-uri — полная цель; ⑩ per-route relaxed CSP — сложность.
**Выбрано:** **④ → ⑨** двухфазно: генерировать nonce в `proxy.ts`, прокинуть в Next (`headers()`/`<Script nonce>`), убрать `unsafe-inline`/`unsafe-eval`, `strict-dynamic`; сохранить `report-uri`; **Trusted Types** — фаза 2. Сначала «наблюдать» → затем «enforce» после evidence.
...
- **`proxy.ts`** — per-request nonce; `x-nonce`+`Content-Security-Policy` в **request**-заголовки (Next `getScriptNonceFromHeader` → auto-nonce bootstrap scripts); CSP на **response** через `applyCsp` на всех ветках возврата.
...
2. **Живой аудит** `scripts/audit-authz-live.mjs` (`pnpm authz:audit`) — интроспекция `information_schema.role_table_grants` × `pg_policies` × `pg_proc`; grant «горит» только когда пермиссивная policy реально пускает роль (иначе инертный дефолт-грант). Прогнан по проду 2026-07-05 → `docs/security/SEC-AUTHZ-GUARD-live-audit.md` (421→20 после фильтрации; genuine follow-up: `profiles`-INSERT).
   ...
   **Follow-up (вне scope шага 10):** column-scope genuine F2-грантов (`profiles`-INSERT verified*\*/itsme*\*/pro_until, purchases/reports status) — отдельная remediation-задача.

### SEC-WAF · Edge/WAF слой (Vercel Firewall) — [P1][⛔]

**Проблема:** application proxy уже выполняет i18n/auth/CSP/access-gate обязанности, но managed edge WAF/bot rules и измеренные cost/rate policies остаются отдельным незакрытым слоем.
...
Исторические ID ниже были импортированы из удалённых legacy trackers и прежнего набора foundations; они сохранены только для трассировки к существующим PRD. Текущий priority/status определяется только V2-разделами этого мастера.
...
### OPS-STAGING · Закрытый production-rehearsal + дисциплина preview-деплоев — [P1][🟡]

Legacy ID сохранён для трассировки. Текущее решение: отдельная hosted staging-среда не обязательна; exact SHA разворачивается в production только за server-side access gate. Чистый replay миграций/грантов выполняется сначала на локальной disposable production-like БД, затем — закрытый founder/team smoke и rollback rehearsal с evidence (учитывая drift-историю [[supabase-migration-drift-repair]]).
...
### LAUNCH-GONOGO · Старый Go/No-Go — superseded

Этот критерий был неполным: он не требовал green CI, E2E, health/readiness, закрытого production rehearsal, restore drill, branch protection и Shannon. Актуальные независимые gates для R0-R4 находятся в [production/RELEASE_GATES.md](production/RELEASE_GATES.md). Contact-only остаётся допустимым только при технически недостижимом money-flow.

---

## Архив V1: приложение A — прежняя сводка P0
