...
README, CLAUDE и все подчинённые PRD обязаны ссылаться сюда. `PROJECT_VISION_AND_TZ` описывает продуктовую идею, `MASTER_TODO` и старые checklists являются историческим каталогом, `docs/features/*` уточняют поведение, но не меняют приоритет или статус из этого мастера.
...
7. атомарный порядок реализации задаёт [aegis/plans/2026-07-10-production-readiness-v2.md](aegis/plans/2026-07-10-production-readiness-v2.md).
...
| ------------------------------ | ----------------------------------------------------------------------- | ------------------------------------------- |
| `R0 Technical Ready`           | Код, админка, staging, CI, E2E, monitoring, recovery и dormant adapters | Нет компании не блокирует                   |
...
| Нет restore drill/staging rehearsal           | Backup/staging были сгруппированы               | Два независимых P0 gates              |
...
- [ ] **P0-01 CI/MAIN.** 🟡 `CODE_COMPLETE`: action SHA исправлены, Gitleaks CLI закреплён с checksum, `main` защищён active ruleset, автоматический Vercel Git deploy для `main` выключен и заменён REST staged→promote workflow после `CI Success` с exact-SHA, project/runtime и incident-race guards. На remote SHA `a8f0d1e` все jobs кроме ожидаемого history secret scan зелёные, Preview прошёл, aggregate gate честно красный. До `VERIFIED` остаются ротация двух Google API keys и общий зелёный CI, Vercel secrets/config + Production promote proof, независимый GitHub reviewer и hotfix drill. Evidence: [production/evidence/P0-01.md](production/evidence/P0-01.md).
...
- [ ] **P0-06 SECURITY.** Enforce CSP, убрать debug/test surfaces, защитить cleanup, закрыть admin/authz gaps.
...
- [ ] **P0-09 DR/STAGING.** Isolated staging, migration rehearsal, DB+Storage restore drill, rollback.
- [ ] **P0-10 DESIGN-FOUNDATION.** Tokens/primitives/dark/reduced-motion/visual/axe без universal AI-patterns.
- [ ] **P0-11 PUBLIC-DESIGN.** Product-first home/card/search/detail/auth и factual trust grammar.
- [ ] **P0-12 PRODUCT-ADMIN-DESIGN.** Post/chat/profile/admin states, dense shell, responsive/accessibility.
...
- [ ] **P0-15 SHANNON.** Разрешённый local/staging proof, reproduce findings, zero confirmed exploitable critical/high.
- [ ] **P0-16 R0-REHEARSAL.** Один SHA, все G0-G14, staging deploy, rollback и signed evidence.
...
- [ ] Twilio SMS/WhatsApp OTP adapters вместо прямых/disabled paths.
- [ ] itsme OIDC и Stripe Identity sandbox-ready adapters.
- [ ] Pro/boost/Stripe Tax test-mode, webhook journal и reconciliation.
...
- [ ] **35.** `OPS-HEALTH` + `OPS-E2E` + `OPS-CI` + `OPS-STAGING` + `OPS-BACKUP` + `OPS-ANALYTICS` — health/uptime + Playwright + CI-гейты + staging + бэкапы/DR + дашборд метрик — 🎚️ **[Opus 4.8 · xhigh]** _(разброс: OPS-ERR/HEALTH — Haiku·low; OPS-BACKUP — Opus·xhigh)_
...
Исторические ID ниже были импортированы из `MASTER_TODO.md` и `FOUNDATIONS-F1-F14`; они сохранены для трассировки к существующим PRD. Текущий priority/status определяется только V2-разделами этого мастера.
