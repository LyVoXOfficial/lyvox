...
- [x] **RULE-01 · RLS даёт ROW, не COLUMN.** Любая user-editable таблица с колонками trust/entitlement/verification/**tax** → column-scoped `GRANT UPDATE (editable cols)` к `authenticated`, revoke table-wide; такие колонки пишет только service-role/SECURITY DEFINER. *(встречено ≥3×: businesses, profiles, +5 в write-lockdown sweep)* → ✅ **автоматизировано шагом 10** (статик-гард `authz-migration-guard.test.ts` RULE-01 + живой аудит `pnpm authz:audit`).
